...
class Q_maze():
    ...
    '''
    def __init__(self, environment, strategy = 'e-greedy', epsilon = 0.1, alpha=0.8, gamma=0.9, game = 'fire-walls', temperature = 1, c = 2, d = 0.8, expression='t^2'):
    ...
    def select_action(self, state):
        ...
        def upper_confidence_bound(self, state, c = 2):
            ...
            if (number_not_chosen_actions > 0):
                ...
                ucb =  q_state + np.sqrt( c * np.log(self.num_episodes) / times_actions[0])
        ...
        def exp3_action_selection(self, state, normalize=True):
            ...
            # Muestrear acción de acuerdo a la distribución generada
            action = np.random.choice(np.arange(self.num_actions), p=list(probabilities))
            return action
    ...
    def train(self, num_episodes):
        ...
        for episode in range(self.num_episodes):
            ...
            while not self.env.terminal_state(state, self.game):
                ...
                q_new = q_old * (1 - alpha) + alpha * (reward + gamma * np.max(self.q_table[next_state]))
...
def plot_steps_per_episode_comp(lista, criteria = 'mean score' , dpi = 100):
    '''
    ...
    for model in lista:
        ...
        if model.strategy in ['softmax']:
            plt.plot(range(model.num_episodes), values, label = model.game + ' | ' + model.strategy + ' |  $\eta = '+ str(model.tau) + '$')
          
        elif model.strategy in ['exp3']:  
            plt.plot(range(model.num_episodes), values, label = model.game + ' | ' + model.strategy + ' |  $\eta = ' + str(model.expression) + '$')
  
        elif model.strategy in ['e-greedy',  'en-greedy']: 
            plt.plot(range(model.num_episodes), values, label = model.game + ' | ' + model.strategy + ' | $\epsilon = ' + str(model.epsilon) + '$')
          
        elif model.strategy in ['UCB1']:  
            plt.plot(range(model.num_episodes), values, label = model.game +' | ' + model.strategy +' | $c = ' + str(model.c) + '$')
